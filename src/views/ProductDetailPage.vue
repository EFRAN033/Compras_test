<template>
            ...
            <div class="space-y-5">
              ...
              <div class="bg-gray-50 p-4 rounded-lg border border-gray-100 shadow-sm">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-4 text-sm text-gray-700">
                    <div v-if="product.proveedor" class="flex items-center">
                      <span class="font-semibold text-gray-900 w-28 flex-shrink-0">Vendido por:</span> 
                      <span class="flex-grow font-bold text-emerald-700">{{ product.proveedor.nombre_empresa }}</span>
                    </div>
              ...
              <div class="mt-8">
                <button @click="requestInventoryReplenishment" class="w-full px-6 py-3 bg-emerald-600 text-white font-semibold rounded-lg shadow-md hover:bg-emerald-700 transition-colors duration-300 text-base transform hover:-translate-y-0.5">
                  Solicitar Llenado de Inventario
                </button>
              </div>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      async requestInventoryReplenishment() {
        const token = localStorage.getItem('authToken');
        const userRole = localStorage.getItem('userRole');
  
        // Solo permitir si hay un token y el rol es 'afiliado'
        if (!token || userRole !== 'afiliado') {
          alert('Debes iniciar sesión como cliente para solicitar un llenado de inventario.');
        ...
        const cantidadDeseada = prompt("¿Qué cantidad desea solicitar al proveedor para este producto? (Ej: 100)");
        
        if (cantidadDeseada === null || cantidadDeseada.trim() === '' || isNaN(cantidadDeseada) || parseInt(cantidadDeseada) <= 0) {
          alert('Solicitud cancelada o cantidad inválida.');
          return;
        }
  
        const data = {
          producto_id: this.product.id,
          cantidad_deseada: parseInt(cantidadDeseada),
        };
        ...
        try {
          ...
          alert(response.data.message || 'Solicitud de llenado de inventario enviada con éxito.');
        } catch (error) {
          console.error('Error al solicitar llenado de inventario:', error.response ? error.response.data : error.message);
          alert('Hubo un error al enviar la solicitud. Detalle: ' + (error.response && error.response.data && error.response.data.detail ? error.response.data.detail : error.message));
        }
      }
    }
  };
