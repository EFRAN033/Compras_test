<template>
  ...
  <script setup>
  import { ref, onMounted, reactive } from 'vue'; // Importar 'reactive'
  ...
  // --- ESTADO REACTIVO ---
  const pendingApplications = ref([]); // Aquí se almacenarán las solicitudes
  const loading = ref(false);
  const notification = reactive({ // Usar reactive para el objeto de notificación
    show: false,
    type: 'success',
    title: '',
    message: '',
  ...
  const getAuthHeaders = () => {
    ...
    if (!token) {
      ...
      router.push('/admin'); // Redirigir al login de admin si no hay token
      return {};
    }
    return {
      headers: {
  ...
  const fetchPendingApplications = async () => {
    ...
    try {
      ...
      if (!headers.headers) return; // Si no hay token, salimos
      
      // Asume que tienes un endpoint en tu backend para obtener solicitudes pendientes
      const response = await axios.get('http://localhost:8000/admin/proveedores/solicitudes/pendientes', headers);
      pendingApplications.value = response.data; // Asigna los datos obtenidos
      ...
      // Si el error es 401 (no autorizado), redirigir al login de admin
  ...
  const approveApplication = async (id) => {
    ...
    try {
      ...
      // Asume que tienes un endpoint en tu backend para aprobar solicitudes
      const response = await axios.post(`http://localhost:8000/admin/proveedores/aprobar/${id}`, {}, headers);
  ...
  const rejectApplication = async (id) => {
    ...
    try {
      ...
      // Asume que tienes un endpoint en tu backend para rechazar solicitudes
      const response = await axios.post(`http://localhost:8000/admin/proveedores/rechazar/${id}`, {}, headers);
  ...
  const logout = () => {
    ...
    router.push('/admin'); // Redirigir al login de administrador
  };
